...
| **CLAIM-7** | The frozen record schema (`validation` feature, Draft 2020-12) rejects a malformed id/type/`createdBy`/signature pattern, a bad attest verdict, and a non-empty tombstone value. | The typed shape and the wire shape can't silently drift apart. | `schema::tests::a_real_claim_and_anchor_validate`, `schema::tests::attestation_value_is_constrained`, `schema::tests::tombstone_must_have_empty_value`, `schema::tests::junk_and_malformed_ids_are_rejected` |
