//! The group identifier — the one type every keyeo layer (both engines + the crypto) names, so an op, a
//! wrap, and a resolved state all agree on which group they belong to.
