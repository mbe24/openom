...
impl Codec for JsonCodec {
    fn parse(&self, bytes: &[u8]) -> Result<ValueTree, CodecError> {
        let mut p = Parser { b: bytes, pos: 0, max_depth: self.max_depth };
        p.ws();
        let v = p.value(0)?;
        p.ws();
        if p.pos != p.b.len() {
            return Err(CodecError::Malformed { pos: p.pos, what: "trailing bytes after value" });
        }
        Ok(v)
    }
...
impl<'a> Parser<'a> {
    ...
    fn lit(&mut self, s: &[u8], val: ValueTree) -> Result<ValueTree, CodecError> {
        if self.b[self.pos..].starts_with(s) {
            ...
            Err(CodecError::Malformed { pos: self.pos, what: "invalid literal" })
        }
    }
    ...
    fn value(&mut self, depth: usize) -> Result<ValueTree, CodecError> {
        ...
        match self.peek() {
            None => Err(CodecError::Malformed { pos: self.pos, what: "unexpected end of input" }),
            ...
            Some(_) => Err(CodecError::Malformed { pos: self.pos, what: "unexpected byte" }),
        }
    }
    ...
    fn number(&mut self) -> Result<ValueTree, CodecError> {
        ...
        let s = std::str::from_utf8(tok).map_err(|_| CodecError::Malformed { pos: start, what: "number" })?;
        if let Some(stripped) = s.strip_prefix('-') {
            // Negative: i64. (`-` alone or `-<huge>` → out of range.)
            let _ = stripped;
            s.parse::<i64>().map(ValueTree::Int).map_err(|_| CodecError::NumberOutOfRange)
        } else {
            s.parse::<u64>().map(ValueTree::Uint).map_err(|_| CodecError::NumberOutOfRange)
        }
    }
    ...
    fn string(&mut self) -> Result<String, CodecError> {
        ...
        loop {
            let c = self.peek().ok_or(CodecError::Malformed { pos: self.pos, what: "unterminated string" })?;
            self.pos += 1;
            match c {
                b'"' => break,
                b'\\' => {
                    let e = self.peek().ok_or(CodecError::Malformed { pos: self.pos, what: "bad escape" })?;
                    self.pos += 1;
                    match e {
                        b'"' => out.push(b'"'),
                        b'\\' => out.push(b'\\'),
                        ...
                        _ => return Err(CodecError::Malformed { pos: self.pos, what: "bad escape" }),
                    }
                }
                0x00..=0x1f => return Err(CodecError::Malformed { pos: self.pos, what: "control char in string" }),
                _ => out.push(c),
            }
        }
        String::from_utf8(out).map_err(|_| CodecError::Malformed { pos: self.pos, what: "invalid utf-8 in string" })
    }

    fn hex4(&mut self) -> Result<u16, CodecError> {
        let s = self.b.get(self.pos..self.pos + 4).ok_or(CodecError::Malformed { pos: self.pos, what: "short \\u escape" })?;
        let s = std::str::from_utf8(s).map_err(|_| CodecError::Malformed { pos: self.pos, what: "\\u escape" })?;
        let v = u16::from_str_radix(s, 16).map_err(|_| CodecError::Malformed { pos: self.pos, what: "\\u hex" })?;
        self.pos += 4;
        Ok(v)
    }
    ...
    fn unicode_escape(&mut self) -> Result<char, CodecError> {
        ...
        if (0xD800..=0xDBFF).contains(&hi) {
            ...
            if self.b.get(self.pos) != Some(&b'\\') || self.b.get(self.pos + 1) != Some(&b'u') {
                return Err(CodecError::Malformed { pos: self.pos, what: "lone high surrogate" });
            }
            self.pos += 2;
            let lo = self.hex4()?;
            if !(0xDC00..=0xDFFF).contains(&lo) {
                return Err(CodecError::Malformed { pos: self.pos, what: "bad low surrogate" });
            }
            let c = 0x10000 + ((hi as u32 - 0xD800) << 10) + (lo as u32 - 0xDC00);
            char::from_u32(c).ok_or(CodecError::Malformed { pos: self.pos, what: "bad surrogate pair" })
        } else if (0xDC00..=0xDFFF).contains(&hi) {
            Err(CodecError::Malformed { pos: self.pos, what: "lone low surrogate" })
        } else {
            char::from_u32(hi as u32).ok_or(CodecError::Malformed { pos: self.pos, what: "bad \\u" })
        }
    }
    ...
    fn array(&mut self, depth: usize) -> Result<ValueTree, CodecError> {
        ...
        loop {
            ...
            match self.peek() {
                ...
                _ => return Err(CodecError::Malformed { pos: self.pos, what: "expected ',' or ']'" }),
            }
        }
    }
    ...
    fn object(&mut self, depth: usize) -> Result<ValueTree, CodecError> {
        ...
        loop {
            self.ws();
            if self.peek() != Some(b'"') {
                return Err(CodecError::Malformed { pos: self.pos, what: "expected object key" });
            }
            let key = self.string()?;
            ...
            self.ws();
            if self.peek() != Some(b':') {
                return Err(CodecError::Malformed { pos: self.pos, what: "expected ':'" });
            }
            self.pos += 1;
            self.ws();
...
fn emit_value(v: &ValueTree, out: &mut Vec<u8>) -> Result<(), CodecError> {
    match v {
        ...
        ValueTree::Bytes(_) => return Err(CodecError::Unrepresentable("raw bytes have no JSON form")),
        ValueTree::Seq(items) => {
            out.push(b'[');
