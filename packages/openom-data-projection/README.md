...
| **PROJ-11** | Forward-compatibility: a claim whose `predicate` this build doesn't recognize is preserved verbatim — attached to `Person.other` when its (effective) target resolves to a person, else carried in `Projection.unclassified` — never dropped, and never mints a spurious person. | New data-model vocabulary is visible (and re-editable) with no projection code change — the read-model half of OPE-212's forward-compat (the mechanism half is `openom-data-crdt`'s CRDT-6). | `tests::unknown_predicates_surface_on_the_person_or_unclassified` |

Run: `node scripts/cargo.mjs test -p openom-data-projection` (from the repo root; on Windows cargo runs under
WSL2/Docker).
