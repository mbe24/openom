...
It is **not** `store_log::DocStore`. `DocStore`'s `append -> seq` (a store-assigned total order) is
...
```rust
use store_blob::{BlobStore, MemoryBlob, Precondition, BlobError};
...
`store_blob::conformance::run(|| MyBackend::new(...))`.
