...
use openom_crypto::{
    derive_root, derive_rvk, generate_dek, generate_hpke_keypair, generate_salt, unwrap_rrk_secret,
    CryptoError, HpkeKeypair, Passphrase, RrkSecret,
};
...
impl DagVault {
    ...
    pub fn unlock(
        ...
        // Fold the effective ops' sealing into the current epochs + escrow.
        let mut epochs: Vec<SealedEpoch> = Vec::new();
        let mut escrow: Option<RecoveryEscrow> = None;
        for bytes in &resolved.sealing {
            let payload: SealingPayload = serde_json::from_slice(bytes)
        ...
        Ok(Unlocked {
            sealer,
            // TODO(OPE-273 anti-rollback): the dag watermark is the frontier op-id set; empty until the
            // sync/anti-rollback wiring lands. Provision+unlock don't exercise it.
            watermark: Vec::new(),
