...
> The app-facing family-tree engine: it composes `openom-data-crdt` (the set-union fold) and
> `openom-data-projection` (the read model) into the read+write surface the app talks to.
...
`openom-data-tree` is the claim-model successor to `openom-treelog`'s engine role — the thing the app's
...
The op semantics + convergence live in `openom-data-crdt` (the `materialize` fold); the read model in
`openom-data-projection`. This crate is the composition and the app-facing surface — nothing more.
