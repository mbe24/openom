...
key. It does no signing or verification itself (that's `openom-data-model` / `openom-crypto`), no I/O,
...
A foundation crate: it depends on no other openom crate, and sits under `openom-data-model` (the
