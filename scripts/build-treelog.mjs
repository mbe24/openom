// Build openom-treelog to WebAssembly — the family-tree engine for the web app. Same two-stage
// flow as build-sealer.mjs (Rust→wasm in Docker because the host can't run cargo build scripts;
// wasm-bindgen glue on the host), just for this crate. Unlike the sealer, treelog + commute use no
// RNG, so no getrandom cfg is needed. Output: apps/app/src/vendor/treelog/ (gitignored).
...
const CRATE = path.join(REPO, 'packages', 'openom-treelog');
...
const CONTAINER_TARGET = `/work/packages/openom-treelog/${TARGET_SUBDIR}`;
const PROFILE = process.env.WASM_PROFILE || 'wasm-release';
const RUSTFLAGS = process.env.WASM_RUSTFLAGS || '';

const WASM = path.join(CRATE, TARGET_SUBDIR, 'wasm32-unknown-unknown', PROFILE, 'openom_treelog.wasm');
...
const PKG_DIR = path.join(REPO, 'apps', 'app', 'src', 'vendor', 'treelog');
...
console.log(`[·] Building openom-treelog → wasm in Docker (wasm32, --features wasm · profile=${PROFILE})…`);
run('docker', [
  'run', '--rm', '--init',
  '-v', `${REPO}:/work`,
  ...
  'rustup target add wasm32-unknown-unknown >/dev/null 2>&1 || true; ' +
    ...
    '-p openom-treelog --no-default-features --features wasm',
]);
